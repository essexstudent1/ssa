...
class Encryption:
    ...
    def encrypt_wake_word(self):
        """Encrypts the wake word entered by using the encryption key from the file, stores the encrypted wake word in a file"""
    ...
    def decrypt_wake_word(self):
        """Decrypts the wake word entered by the user using the encryption key from the file"""
        with open(self.key_file, "rb") as filekey:
            key = filekey.read()
        fernet = Fernet(key)
...
class WakeUpHub:
    """A class used to wake up the hub, create a wake word and authenticate it"""
    def __init__(self):
        self.file = "wake_word.txt"

    def auth_wake_word(self, word):
        """Decrypts the wake word set up and stored in a file and compares it to the wake word entered by the user"""
        encr = Encryption()
    ...
    def create_wake_word(self):
        """Asks the user to input wake word, encrypts it and stores it in a file"""
    ...
    def wake_up(self):
        """If the wake word file is not 0 (wake word exists), asks the user to input the wake word. Otherwise, asks the user to create a wake word"""
...
def broker_auth():
    """Opens the password file where the controller hub user name and the password are stored for accessing the MQTT broker"""
...
mqttBroker = "mqtt.aliahmed.app" # Identifies the MQTT broker location

client = mqtt.Client("Hub-984323") # Sets up the new client
client.username_pw_set(broker_auth()[0], broker_auth()[1]) # Passes the client credentials to the MQTT broker
client.connect(mqttBroker) # Tells the client to connect to the broker
...
# Defines a loop which allows the user to lock/unlock the door without having to enter the wake word each time if the wake word has been authenticated
while True:
    if wakeup_test_complete == False:
        user_interface(True)
